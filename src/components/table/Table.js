...
const Table = () => {
  ...
  const elements = wordList.map((word) => (
    <Row key={word.id} {...word} onDelete={showConfirm} />
  ));

  const filteredElements =
    term === ""
      ? elements
      : elements.filter((element) => element.props.tags === term);
  const currentWords = filteredElements.slice(
    indexOfFirstWord,
    indexOfLastWord
  );
